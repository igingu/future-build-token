...
import { DynamicStaking, DynamicStaking__factory, FutureBuildToken, FutureBuildToken__factory } from "../../misc/typechain-types";
...
describe("DynamicStaking", () => {
	...
	before(async () => {
		...
		dynamicStakingFactory = await ethers.getContractFactory(Constants.ContractNames.DynamicStaking) as DynamicStaking__factory;
	...
	it("Should allow you to stake", async () => {
		await expect(futureBuildToken.approve(dynamicStaking.address, 1000)).to.not.be.reverted;
